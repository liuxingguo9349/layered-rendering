...
for epoch in range(num_epochs):
    for i, mini_batch in enumerate(train_loader):
        ...
        if i % 200 == 0:
            y = fake_img.data
            fid_cur =0
            torch.save(y, f"/content/drive/MyDrive/2023_4_!/haiyang/input_{len(train_loader) * epoch + i}.pt")
            if(fid_cur/40) < fid_min:
                fid_min = fid_cur/40
                torch.save(copy.deepcopy(G.state_dict()),
                           f"/content/drive/MyDrive/2023_4_!/para/G_model_{len(train_loader) * epoch + i}_fid:{fid_min}.pth")
