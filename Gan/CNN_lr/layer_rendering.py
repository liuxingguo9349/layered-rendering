    ...
    for y in range(1, 13):
        for i in range(100, 201, 10):
            ...
            model_P.load_state_dict(
                torch.load(f"J:/CNN/mon{x}/mon{y}/best_model.pth", map_location=torch.device('cpu')))
            model_G.load_state_dict(
                torch.load('G:/G_model_2560_fid_15.337425305337948.pth', map_location=torch.device('cpu')))
            ...
            res = model_G(noise).clone().detach()
            print(res.size())
            torch.save(res, f"J:/res/target_{x}/mon_{y}/{y}mon{x}_best_layered_rendering_{tar}.pt")
