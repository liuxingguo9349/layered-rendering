...
fid_example= np.array(inpv1[0:40,:,:,:])
fid_min = 1000
...
class WGAN_G(nn.Module):
    def __init__(self):
        ...
        # self.cbam1 = CBAM(128)
        self.gam1 = GAM_Attention(in_channels=128, out_channels=128)
        self.deconv2 = nn.ConvTranspose2d(128, 64, 4, 2, 1)
        self.deconv2_bn = nn.BatchNorm2d(64)
        # self.cbam2 = CBAM(64)
        self.gam2 = GAM_Attention(in_channels=64, out_channels=64)
        self.deconv3 = nn.ConvTranspose2d(64, 6, 4, 2, 1)
        # self.cbam3 = CBAM(6, ratio=2)
        self.gam3 = GAM_Attention(in_channels=6, out_channels=6, rate=2)
        self.relu = nn.ReLU()
    ...
    for i, mini_batch in enumerate(train_loader):
        ...
        if i % 200 == 0:
            ...
            for k in range(40):
                for j in range(6):
                    act1 = fake_res[k, j, :, :]
                    act2 = fid_example[k, j, :, :]
                    fid_cur += calculate_fid(act1, act2)
            if (fid_cur / 240) < fid_min:
                fid_min = fid_cur / 240
                torch.save(copy.deepcopy(G.state_dict()),
                           f"/home/t_gan/res/model1/gam_model_{len(train_loader) * epoch + i}_fid:{fid_min}.pth")
